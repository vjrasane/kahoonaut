...
<div class="h-100 flex-col">
	...
	<div class="mt-3 gap-1 d-flex justify-start">
		<button class="btn btn-primary w-100" on:click={onAdd}>Add</button>
		<button class="btn btn-secondary w-100" disabled={!$questions.length} on:click={onShuffle}
			>Shuffle Answers</button
		>
		<button class="btn btn-success w-100" disabled={!$questions.length} on:click={onExport}
			>Export</button
		>
	</div>
	<hr />
